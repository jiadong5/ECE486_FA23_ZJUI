...
% 95-105% bounds.
x = length(y)-1; %initialize x to the end of the array
while all(y(x:length(y)) > 0.95 * ssVal) && all(y(x:length(y)) < 1.05 * ssVal)%PLACE YOUR CONDITIONS HERE
    x = x-1;
end
...
% Label the axes:
ylabel('response to step in referance');
xlabel('t, time');
